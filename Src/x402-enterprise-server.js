        ...
        const paymentRequest = {
            ...
            payment: {
                ...
                schemes: allowDeferred !== false ? [
                    ...
                    { type: 'privacy', currencies: ['usdc', 'zec'], verification: 'on-chain', enabled: CONFIG.PRIVACY_ENABLED }
                ] : [
                    { type: 'immediate', currencies: currencies || ['usdc', 'zec', 'xmr'], verification: 'on-chain' },
                    { type: 'privacy', currencies: ['usdc', 'zec'], verification: 'on-chain', enabled: CONFIG.PRIVACY_ENABLED }
                ]
            ...
            expiresIn: 3600,
            instructions: {
                usdc: `Send ${amount} USDC to ${CONFIG.PAYMENT_WALLET} on Base, Polygon, or StarkNet, provide tx hash`,
...
app.get('/api/v1/payments/deferred/balance/:clientId', async (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            settlementNetworks: ['base', 'polygon', 'starknet']
        });
    } catch (error) {
...
app.get('/api/v1/admin/dashboard', authenticateAdmin, requirePermission('dashboard:read'), auditMiddleware('dashboard:read', '/api/v1/admin/dashboard'), async (req, res) => {
    try {
        ...
        const privacyRoutedPayments = await Payment.count({ where: { privacyRouted: true } });
        ...
        const activePrivacyRoutes = await PrivacyRoute.count({
            where: {
                stage: {
                    [Op.not]: ['completed', 'failed']
                }
            }
        });
    ...
    try {
        ...
        res.json({
            payments: rows.map(p => ({
                ...
                metadata: p.metadata,
                privacyRouted: p.privacyRouted,
                privacyRouteId: p.privacyRouteId,
                createdAt: p.createdAt,
                confirmedAt: p.confirmedAt,
...
auditMiddleware('payments:read', '/api/v1/admin/payments/:id'), async (req, res) => {
    try {
        ...
        let privacyRoute = null;
        if (payment.privacyRouteId) {
            privacyRoute = await PrivacyRoute.findByPk(payment.privacyRouteId);
        }
        ...
        res.json({
            payment: {
                ...
                privacyRouted: payment.privacyRouted,
                privacyRouteId: payment.privacyRouteId,
            ...
            privacyRoute: privacyRoute ? {
                id: privacyRoute.id,
                stage: privacyRoute.stage,
                sourceChain: privacyRoute.sourceChain,
                sourceTxHash: privacyRoute.sourceTxHash,
                zecDepositTx: privacyRoute.zecDepositTx,
                zecShieldedTx: privacyRoute.zecShieldedTx,
                zecIntermediateTx: privacyRoute.zecIntermediateTx,
                zecUnshieldTx: privacyRoute.zecUnshieldTx,
                destinationChain: privacyRoute.destinationChain,
                destinationTxHash: privacyRoute.destinationTxHash,
                mixingDelaySeconds: privacyRoute.mixingDelaySeconds,
                hopCount: privacyRoute.hopCount,
                privacyScore: privacyRoute.privacyScore,
                errorMessage: privacyRoute.errorMessage,
                retryCount: privacyRoute.retryCount,
                depositedAt: privacyRoute.depositedAt,
                shieldedAt: privacyRoute.shieldedAt,
                unshieldedAt: privacyRoute.unshieldedAt,
                completedAt: privacyRoute.completedAt
            } : null
        });
    } catch (error) {
...
app.get('/api/v1/admin/privacy-routes', authenticateAdmin, requirePermission('payments:read'), 
auditMiddleware('privacy:read', '/api/v1/admin/privacy-routes'), async (req, res) => {
    try {
        ...
        const { count, rows } = await PrivacyRoute.findAndCountAll({
            where,
            limit: parseInt(limit),
            offset: parseInt(offset),
            order: [['createdAt', 'DESC']],
            include: [{
                model: Payment,
                attributes: ['id', 'clientId', 'amount', 'currency', 'status']
            }]
        });
        
        res.json({
            privacyRoutes: rows.map(route => ({
                id: route.id,
                paymentId: route.paymentId,
                sourceChain: route.sourceChain,
                sourceTxHash: route.sourceTxHash,
                sourceAmount: parseFloat(route.sourceAmount),
                sourceCurrency: route.sourceCurrency,
                stage: route.stage,
                zecDepositTx: route.zecDepositTx,
                zecShieldedTx: route.zecShieldedTx,
                zecIntermediateTx: route.zecIntermediateTx,
                zecUnshieldTx: route.zecUnshieldTx,
                destinationChain: route.destinationChain,
                destinationTxHash: route.destinationTxHash,
                mixingDelaySeconds: route.mixingDelaySeconds,
                hopCount: route.hopCount,
                privacyScore: route.privacyScore,
                errorMessage: route.errorMessage,
                retryCount: route.retryCount,
                depositedAt: route.depositedAt,
                shieldedAt: route.shieldedAt,
                scheduledUnshieldAt: route.scheduledUnshieldAt,
                unshieldedAt: route.unshieldedAt,
                completedAt: route.completedAt,
                payment: route.Payment ? {
                    id: route.Payment.id,
                    clientId: route.Payment.clientId,
                    amount: parseFloat(route.Payment.amount),
                    currency: route.Payment.currency,
                    status: route.Payment.status
                } : null
            })),
            pagination: {
                page: parseInt(page),
        ...
        logger.error('Privacy routes list error:', error);
        res.status(500).json({ error: 'Failed to fetch privacy routes' });
    }
});
...
auditMiddleware('config:read', '/api/v1/admin/config'), (req, res) => {
    res.json({
        ...
        privacyRouter: {
            enabled: CONFIG.PRIVACY_ENABLED,
            minDelay: CONFIG.PRIVACY_MIN_DELAY,
            maxDelay: CONFIG.PRIVACY_MAX_DELAY,
            multiHop: CONFIG.PRIVACY_MULTI_HOP
        },
        supportedCurrencies: ['USDC', 'ZEC', 'XMR'],
        supportedNetworks: ['base', 'polygon', 'starknet', 'zcash', 'monero'],
        depositWallets: {
            usdc_base: CONFIG.PAYMENT_WALLET,
            usdc_polygon: CONFIG.PAYMENT_WALLET,
            usdc_starknet: CONFIG.STARKNET_ACCOUNT_ADDRESS,
    ...
    res.json({
        ...
        endpoints: {
            ...
            payment: {
                ...
                privacy: {
                    route: 'POST /api/v1/payments/privacy-route',
                    status: 'GET /api/v1/payments/privacy-route/:routeId/status'
                }
            },
            zcash: {
            ...
            admin: {
                ...
                privacyRoutes: 'GET /api/v1/admin/privacy-routes',
